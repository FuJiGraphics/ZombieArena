#include "stdafx.h"
#include "SceneGameScores.h"

SceneGameScores::SceneGameScores()
	: Scene(SceneIds::SceneGameScores)
{
}

void SceneGameScores::Init()
{
	Scene::Init();
	...
	{
		items.push_back(resultData.top());
		resultData.pop();
	}
...
void SceneGameScores::Release()
{
	Scene::Release();
}

void SceneGameScores::Enter()
{
...
void SceneGameScores::Draw(sf::RenderWindow& window)
{
...
void SceneGameScores::Update(float dt)
{
	Scene::Update(dt);
	if (InputMgr::GetMouseButtonDown(sf::Mouse::Button::Left))
	{
		SCENE_MGR.ChangeScene(nextScene, SceneIds::SceneGameScores);
		return;
	}
}
