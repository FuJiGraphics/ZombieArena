...
sf::Vector2f Utils::RandomDirection(float min, float max, const sf::Vector2f& base)
{
    return Utils::GetRotateVector(Utils::RandomRange(min, max), base);
}
...
sf::Vector2f Utils::SetOrigin(TransformComponent& obj, Origins preset, const sf::FloatRect rect)
{
    sf::Vector2f newOrigin(rect.width, rect.height);
    newOrigin.x *= ((int)preset % 3) * 0.5f;
    newOrigin.y *= ((int)preset / 3) * 0.5f;
    obj.Origin.translate(newOrigin * -1.0f);
    return newOrigin;
}
...
sf::Vector2f Utils::SetOrigin(TileMap& obj, Origins preset)
{
    sf::Vector2f newOrigin(obj.GetLength().x, obj.GetLength().y);
    newOrigin.x *= ((int)preset % 3) * 0.5f;
    newOrigin.y *= ((int)preset / 3) * 0.5f;
    return newOrigin;
}
